...
Application.prototype.describe = function(collectionName, cb) {
  ...
  this.getCollection(collectionName, function(err, data) {
    ...
    var schemas = Object.keys(data.schemas).length > 0 ? data.schemas : null;
    cb(null, schemas);
  });
};
...
Application.prototype.destroy = function(pathname, options, cb) {
  ...
  const onComplete = function(count) {
    return count - resultSet.results.length;
  };

  const callable = (resolve, reject) => {
    self.data[pathname] = _.reject(this.data[pathname], function (model, i) {
      return _.includes(resultSet.indices, i);
    });
  };
  self.database.ref('counts').child(pathname).transaction(onComplete)
    .catch(cb);
  //
  ...
  // console.log('RESULT SET');
  // console.log('RESULT SET');
  // console.log(resultSet.results.length);
  ...
  // const promises = [
  //   self.database.ref('schemas').child(pathname).set(null)
  // ];
