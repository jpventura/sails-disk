...
const Promise = require('firebase-admin').Promise;
const Aggregate = require('./aggregates');
const async = require('async');
const errors = require('waterline-errors');
...
Application.prototype.write = function(collectionName, cb) {
  ...
  console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>>')
  console.log(Object.keys(this.collections[collectionName]).length);
  console.log('<<<<<<<<<<<<<<<<<<<<<<<<<<<');
  console.log(' ');
  console.log(' ');
  console.log(' ');
  ...
  return self.database.ref('data').child(collectionName).set(xxx).then(onResolve, onReject);
  // if (!this.data[collectionName]) {
  //   return Promise.all([
  //     self.database.ref('data').child(collectionName).update(xxx),
  ...
  // this.diane(collectionName, porra, function(error, datum) {
  //   console.log(datum);
  // });
};
