...
const ChildEventListener = function(reference) {
  this.reference = reference;
  this.callbacks = [];
};

ChildEventListener.promises.off = function off(cb) {
  _.remove(this.callbacks, function(callback) {
    return callback === cb;
  });

  if (this.callbacks.length === 0) {
    this.reference.off('child_added', this.onAdded, this);
    this.reference.off('child_changed', this.onChanged, this);
    this.reference.off('child_moved', this.onMoved, this);
    this.reference.off('child_removed', this.onRemoved, this);
  }
};

ChildEventListener.promises.on = function on(cb) {
  this.callbacks.push(cb);
  this.reference.on('child_added', this.onAdded, this.onCanceled, this);
  this.reference.on('child_changed', this.onChanged, this.onCanceled, this);
  this.reference.on('child_moved', this.onMoved, this.onCanceled, this);
  this.reference.on('child_removed', this.onRemoved, this.onCanceled, this);
};

ChildEventListener.prototype.onAdded = function onAdded(snapshot, previousChildName) {
  return this.publishUpdate('added', snapshot.key, snapshot.val());
};

ChildEventListener.prototype.onCanceled = function onCanceled(error) {
  console.error(error);
};

ChildEventListener.prototype.onChanged = function onChanged(snapshot, previousChildName) {
  return this.publishUpdate('updated', snapshot.key, snapshot.val());
};

ChildEventListener.prototype.onMoved = function onMoved(snapshot, previousChildName) {
  this.publishUpdate('updated', snapshot.key, snapshot.val());
};

ChildEventListener.prototype.onRemoved = function onRemoved(snapshot, previousChildName) {
  this.publishUpdate('destroyed', snapshot.key, snapshot.val());
};

ChildEventListener.prototype.publishCreated = function publishCreated(record) {
  const event = {
    verb: 'created',
    id: record.id,
    data: record
  };
  return this.callbacks.map(cb);
};

ChildEventListener.prototype.publishDestroyed = function publishDestroyed(record) {
  const event = {
    verb: 'destroyed',
    id: record.id,
    data: record
...
ChildEventListener.prototype.publishUpdate = function publishUpdate(record) {
  const event = {
    verb: 'updated',
    id: record.id,
    data: record
...
Application.prototype.insert = function(collectionName, values, cb) {
  ...
  try {
    self.create(collectionName, values)
...
Application.prototype.update2 = function(collectionName, options, novo, cb) {
  var self = this;
  ...
  for (var i in resultSet.indices) {
    ...
    self.data[collectionName][matchIndex] = _.extend(_values, novo);
 ...
 *
 * @param {String} pathname
 * @param {Object} one or more collection records
 * @return {Object}
 * @api public
 */
Application.prototype.create = function(pathname, collection) {
  const self = this;
  const data = self.database.ref('data').child(pathname);
  const promises = [];
  const records = [];
  ...
  const createOne = function(record) {
    const serializeRecordIntoDocument = function(record) {
      return self._serialize(pathname, record);
    };

    const definePrimaryKeyIfRequired = function(record) {
      if (!record.id) {
        record.id = data.push().key;
      }
    ...
    const createDocument = function(document) {
      return data.child(document.id).set(document).then(function() {
        return record;
      });
    };
    ...
    return Promise.resolve(_.cloneDeep(record))
      .then(serializeRecordIntoDocument)
      .then(definePrimaryKeyIfRequired)
      .then(createDocument)
      .then(pushRecord);
  };

  collection.forEach(createOne);
...
Application.prototype.define = function(pathname, definition) {
  ...
  return self.database.ref('schemas').child(definition).set(definition)
};
...
Application.prototype.destroy = function(pathname, options) {
  ...
  const destroyOne = function(record) {
  ...
  const destroyFound = function(documents) {
    const promises = [];

    documents.forEach(function(document) {
      promises.push(Promise.resolve(document).then(destroyOne));
    });
  ...
  return self.find(pathname, options).then(destroyFound);
};
...
Application.prototype.find = function(pathname, options) {
  ...
  const turnSnapshotIntoArray = function(snapshot) {
  ...
  if (!options) {
    return reference.once('value').then(turnSnapshotMapIntoArray);
  }

  return reference.once('value')
    .then(turnSnapshotIntoArray)
    .then(filter)
    .then(aggregate);
};

Application.prototype.on = function(identity, pathname, cb) {
  const self = this;

  if (!self.schema[pathname]) {
    const error = new Error(WaterlineErrors.CollectionNotRegistered);
    return cb(error);
  }

  const reference = self.database.ref('data').child(pathname);
  const listener = ChildEventListener(cb);

  reference.on('child_added', listener.onAdd, listener.onCancel, listener);
  reference.on('child_changed', listener.onChange, listener.onCancel, listener);
  reference.on('child_moved', listener.onMove, listener.onCancel, listener);
  reference.on('child_removed', listener.onRemove, listener.onCancel, listener);

  return cb();
};

Application.prototype.off = function(identity, pathname, cb) {
  return cb();
};

Application.prototype.update = function(pathname, options, values) {
  const self = this;

  const updateMatchingCollection = function(collection) {
    const documents = [];

    collection.forEach(function(record){
      documents.push(_.extend(record, _.cloneDeep(values)));
    });

    return documents;
  };

  return self.find(pathname, options)
    .then(updateMatchingCollection);
};
