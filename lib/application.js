...
//   const counts = self.database.ref('counts').child(pathname);
  ...
  const createRecord = function(record) {
    ...
    const serializeRecordToDocument = function(record) {
    ...
    // const incrementCount = function(record) {
    //   const transactionUpdate = function(count) {
    //     return count + 1;
    //   };
    //
    //   return counts.transaction(transactionUpdate).then(function() {
    //     return record;
    //   });
    // };
    ...
    return Promise.resolve(_.cloneDeep(record))
      .then(serializeRecordToDocument)
  ...
  collection.forEach(createRecord);

  const resolveAll = function() {
    return records;
  };

  return Promise.all(promises).then(resolveAll);
};
...
Application.prototype.find = function(pathname, options) {
  ...
  const snapshotToArray = function snapshotToArray(snapshot) {
    const collection = [];
  ...
  if (!options) {
    return reference.once('value').then(snapshotToArray);
  }
