...
Application.prototype.read = function(cb) {
  ...
  for (var collectionName in state.data) {
    state.data[collectionName] = this._unserializeDocument(collectionName, state.data[collectionName]);
  }
 ...
 * @api public
 */
Application.prototype.describe = function(pathname, cb) {
  const self = this;

  const onResolve = function(snapshot) {
    return cb(null, snapshot.exists() ? snapshot.val() : null);
  };
  ...
  return self.database.ref('schemas')
    .child(pathname)
    .once('value')
    .then(onResolve)
    .catch(onReject);
...
Application.prototype.insert = function(collectionName, values, cb) {
  ...
  // console.log('comprimento ' + values.length);
...
Application.prototype.update = function(collectionName, options, values, cb) {
  ...
  var pkAttrName = getPrimaryKey(this.schemas[collectionName]);
...
Application.prototype.destroy = function(pathname, options, cb) {
  const self = this;
...
Application.prototype.serializeValues = function(collectionName, values) {
  ...
  Object.keys(values).forEach(function(key) {
    ...
    if(!self.schemas[collectionName]) return;
    if(!self.schemas[collectionName].hasOwnProperty(key)) return;

    var type = self.schemas[collectionName][key].type;
    var val;
...
Application.prototype._serializeDocument = function serializeDocument(pathname, collection) {
  const self = this;
};

Application.prototype._unserializeDocument = function unserializeDocument(pathname, collection) {
  const self = this;

  const unserialize = function(document) {
    return self.collections[pathname]._cast.run(document);
  };

  return collection.map(unserialize);
};

Application.prototype.onGetCollection = function onGetCollection(snapshot) {
  const collection = [];
...
Application.prototype._destroyRecords = function(pathname, collections, options) {
  ...
  for (var i = 0; i < query.results.length; i++) {
    promises.push(data.child(query.results[i].id).set(null));
  }

  const updateRecordCount = function(count) {
    return count - query.results.length;
  };

  return Promise.all(promises).then(counts.transaction(updateRecordCount));
};
...
Application.prototype.find = function(pathname, options) {
  ...
  if (!options) {
    return reference.once('value').then(this.onGetCollection);
  }
