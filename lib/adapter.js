...
module.exports = (function () {
  ...
  var adapter = {
    ...
    update: function (conn, coll, options, values, cb) {
      grabConnection(conn).update2(coll, options, values, cb);
    },
  ...
  // http://sailsjs.com/documentation/reference/web-sockets/sails-sockets
  // 
  on: function on(conn, coll, cb) {
    return grabConnection(conn).on(conn, coll, cb);
  };

  // http://sailsjs.com/documentation/reference/web-sockets/sails-sockets
  // http://sailsjs.com/documentation/concepts/realtime
  off: function on(conn, coll, cb) {
    return grabConnection(conn).off(conn, coll, cb);
  };
